...
import com.example.AddressJoin.mapper.CountryMasterMapper;
...
public class AddressServiceImpl implements AddressService {
    ...
    public AddressApiResponse<CountryMasterDTO> getCountryWithStatesDistrictsSubDistricts(Long countryId) {
        log.info("Fetching country with states, districts, and sub-districts for country ID: {}", countryId);

        List<CountryMaster> countryEntities = addressQueryBuilderRepository.findDynamicAddresses(
                List.of(countryId), null, null, null);

        if (countryEntities.isEmpty()) {
            return new AddressApiResponse<>(null, null, HttpStatus.NOT_FOUND, "No data found", true);
        }

        // Convert entities to DTOs
        List<CountryMasterDTO> countryDTOList = countryMasterMapper.toDTOList(countryEntities);

        return new AddressApiResponse<>(null, countryDTOList, HttpStatus.OK, "Data fetched successfully", false);
    }

    @Override
    public AddressApiResponse<List<CountryMasterDTO>> getFilteredAddresses(
            List<Long> countryIds, List<Long> stateIds, List<Long> districtIds, List<Long> subDistrictIds) {
        log.info("Fetching filtered addresses...");

        List<CountryMaster> countryEntities = addressQueryBuilderRepository.findDynamicAddresses(
                countryIds, stateIds, districtIds, subDistrictIds);
        ...
        return new AddressApiResponse<>(countryDTOList, null, HttpStatus.OK, "Filtered data fetched successfully", false);
    }
